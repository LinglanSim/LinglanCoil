...
namespace Model
{
    public class CheckAirConvergeforCircuits
    {
        public static CheckAir CheckAirConverge(int[,] TotalDirection, int Nrow, int N_tube, int Nelement, double[, ,] tain, double[, ,] RHin, CalcResult[] r)//, double[, ,] taout, double[, ,] RHout)
        {
            ...
            //to be modified..
            if (index == 0) flag = true; //&& (r[r.Count() - 1].x_o > 1 || r[r.Count() - 1].x_o < 0)) 
            else
            {
                tain = temp;
                RHin = temp1;
        ...
        public static CheckAir CheckAirConverge2(int[,] TotalDirection, int Nrow, int N_tube, int Nelement, double[, ,] tain, double[, ,] RHin, double[, ,] taout, double[, ,] RHout)//, double[, ,] taout, double[, ,] RHout)
        {
            CheckAir rr = new CheckAir();
            int index = 0;
            bool flag = false;
