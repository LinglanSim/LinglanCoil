        ...
        public double Interpolate(int FirstRow, double FirstRowValue, double FirstColValue, double X, double Y, double deta_X, double deta_Y, int dig_X, int dig_Y, double[,] SourceTableData)
        {
            ...
            if (X != Math.Round(X, dig_X) && Y != Math.Round(Y, dig_Y))
            {
                if (X < Math.Round(X, dig_X))
                {
                    ...
                    if (Y > Math.Round(Y, dig_Y))
                    {
                        ...
                        double X1Y1 = Convert.ToDouble(SourceTableData[rowNum, colNum]);
                        double X2Y1 = Convert.ToDouble(SourceTableData[rowNum1, colNum]);
                        double X1Y2 = Convert.ToDouble(SourceTableData[rowNum, colNum1]);
                        double X2Y2 = Convert.ToDouble(SourceTableData[rowNum1, colNum1]);
                        Z1 = X1Y1 + (X2Y1 - X1Y1) / deta_X * (SourceTableData[rowNum, 0] - X);
                        Z2 = X1Y2 + (X2Y2 - X1Y2) / deta_X * (SourceTableData[rowNum, 0] - X);
                        //Z = Z1 + (Z2 - Z1) / deta_Y * (Y - Convert.ToDouble(Model.HumidAirSourceData.SourceTableData[FirstRow - 1, colNum]));//0.05是表纵坐标相邻间距
                        Z = Z1 + (Z2 - Z1) / deta_Y * (Y - SourceTableData[FirstRow, colNum]);//0.05是表纵坐标相邻间距
                    }
                    else
                    {
                        colNum1 = colNum - 1;
                        double X1Y1 = SourceTableData[rowNum, colNum];
                        double X2Y1 = SourceTableData[rowNum1, colNum];
                        double X1Y2 = SourceTableData[rowNum, colNum1];
                        double X2Y2 = SourceTableData[rowNum1, colNum1];
                        Z1 = X1Y1 + (X2Y1 - X1Y1) / deta_X * (SourceTableData[rowNum, 0] - X);
                        Z2 = X1Y2 + (X2Y2 - X1Y2) / deta_X * (SourceTableData[rowNum, 0] - X);
                        //Z = Z1 + (Z2 - Z1) / deta_Y * (Convert.ToDouble(Model.HumidAirSourceData.SourceTableData[FirstRow - 1, colNum]) - Y);//0.05是表纵坐标相邻间距
                        Z = Z1 + (Z2 - Z1) / deta_Y * (SourceTableData[FirstRow, colNum] - Y);//0.05是表纵坐标相邻间距
                    }
                ...
                else
                {
                    ...
                    if (Y > Math.Round(Y, dig_Y))
                    {
                        ...
                        double X1Y2 = SourceTableData[rowNum, colNum1];
                        double X2Y2 = SourceTableData[rowNum1, colNum1];
                        Z1 = X1Y1 + (X2Y1 - X1Y1) / deta_X * (X - SourceTableData[rowNum, 0]);
                        Z2 = X1Y2 + (X2Y2 - X1Y2) / deta_X * (X - SourceTableData[rowNum, 0]);
                        //Z = Z1 + (Z2 - Z1) / deta_Y * (Y - Convert.ToDouble(Model.HumidAirSourceData.SourceTableData[FirstRow - 1, colNum]));//0.05是表纵坐标相邻间距
                        Z = Z1 + (Z2 - Z1) / deta_Y * (Y -SourceTableData[FirstRow, colNum]);//0.05是表纵坐标相邻间距
                    }
                    else
            ...
            else if (X != Math.Round(X, dig_X) && Y == Math.Round(Y, dig_Y))
            {
                if (X < Math.Round(X, dig_X))
                {
                    ...
                    double X1Y1 =SourceTableData[rowNum, colNum];
                    double X2Y1 = SourceTableData[rowNum1, colNum];
                    Z1 = X1Y1 + (X2Y1 - X1Y1) / deta_X * (SourceTableData[rowNum, 0] - X);//0.05是表纵坐标相邻间距
                }
                else
                {
                    rowNum1 = rowNum + 1;
                    double X1Y1 =SourceTableData[rowNum, colNum];
                    double X2Y1 = SourceTableData[rowNum1, colNum];
                    Z1 = X1Y1 + (X2Y1 - X1Y1) / deta_X * (X - SourceTableData[rowNum, 0]);//0.05是表纵坐标相邻间距
                }
                Z = Z1;
            }
            else if (X == Math.Round(X, dig_X) && Y != Math.Round(Y, dig_Y))
            {
                if (Y > Math.Round(Y, dig_Y))
                {
                    ...
                    Z1 = X1Y1 + (X1Y2 - X1Y1) / deta_Y * (Y - SourceTableData[FirstRow, colNum]);//0.0002是表横坐标相邻间距
                }
                else
                {
                    colNum1 = colNum - 1;
                    ...
                    Z1 = X1Y1 + (X1Y2 - X1Y1) / deta_Y * (SourceTableData[FirstRow, colNum] - Y);//0.0002是表横坐标相邻间距
                }
            ...
            {
                Z = SourceTableData[rowNum, colNum];
            }

            return Z;
        }

        public double Interpolate_Ts(int FirstRow, double X, double deta_X, int dig_X, double[,] SourceTableData)
        {
            double Z1;
            double Z;
